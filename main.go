...
func main() {
	...
	// Static files and API endpoints with authentication
	wrapHandleWithAuth(*listenBaseUrl, http.StripPrefix(strings.TrimRight(*listenBaseUrl, "/"), static))
	http.HandleFunc(*listenBaseUrl+"api/server/settings", wrapWithAuth(ovpnAdmin.serverSettingsHandler))
	http.HandleFunc(*listenBaseUrl+"api/users/list", wrapWithAuth(ovpnAdmin.userListHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/create", wrapWithAuth(ovpnAdmin.userCreateHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/change-password", wrapWithAuth(ovpnAdmin.userChangePasswordHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/rotate", wrapWithAuth(ovpnAdmin.userRotateHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/delete", wrapWithAuth(ovpnAdmin.userDeleteHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/revoke", wrapWithAuth(ovpnAdmin.userRevokeHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/unrevoke", wrapWithAuth(ovpnAdmin.userUnrevokeHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/config/show", wrapWithAuth(ovpnAdmin.userShowConfigHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/mfa", wrapWithAuth(ovpnAdmin.userShowMFAHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/disconnect", wrapWithAuth(ovpnAdmin.userDisconnectHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/statistic", wrapWithAuth(ovpnAdmin.userStatisticHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/ccd", wrapWithAuth(ovpnAdmin.userShowCcdHandler))
	http.HandleFunc(*listenBaseUrl+"api/user/ccd/apply", wrapWithAuth(ovpnAdmin.userApplyCcdHandler))

	http.HandleFunc(*listenBaseUrl+"api/sync/last/try", wrapWithAuth(ovpnAdmin.lastSyncTimeHandler))
	http.HandleFunc(*listenBaseUrl+"api/sync/last/successful", wrapWithAuth(ovpnAdmin.lastSuccessfulSyncTimeHandler))
	http.HandleFunc(*listenBaseUrl+downloadCertsApiUrl, wrapWithAuth(ovpnAdmin.downloadCertsHandler))
	http.HandleFunc(*listenBaseUrl+downloadCcdApiUrl, wrapWithAuth(ovpnAdmin.downloadCcdHandler))
